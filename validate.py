...
def validate_info(main,filename):
    ...
    for key,value in informatii.items():
        b=ttk.Button(img_frame,text=key,command=partial(crop.decupeaza,canvas,img_copy,filename,key,informatii))
        b.grid(column=1,row=current_row)
        b_valid=ttk.Entry(img_frame,width=lung_min[key],justify='center',font=('calibre',10,'normal'),textvariable=informatii[key])
        b_valid.grid(column=2,row=current_row)
        current_row+=1
        
    valid_button=ttk.Button(img_frame,text="Informatii valide", image=photoimg_ok1, compound=LEFT, command=partial(ok_validate,img_frame,informatii))
    valid_button.grid(column=1,row=9)
    cancel_button=ttk.Button(img_frame,text="Inchide", image=photoimg_nok1, compound=LEFT, command=partial(nok_validate,img_frame))
    cancel_button.grid(column=2,row=9)


def ok_validate(img_frame,informatii):
    ok = messagebox.askokcancel(title="Informatii validate",message="S-au salvat informatiile.\nApasati pe Ok pentru a continua")
    if ok:
        img_frame.destroy()
        img_frame.update()
    else:
...
def nok_validate(img_frame):
    global informatii
    retry = messagebox.askretrycancel(title="Informatii nevalidate",message="Nu veti putea completa documentele.\nApasati pe Retry pentru a va intoarce")
    if retry:
        pass
    else:
        informatii.clear()
